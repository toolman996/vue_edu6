<template>
    <div class="cart">
        ...
        <div class="cart-info">
            ...
            <div class="cart-item" v-for="value in course">
                <el-row>
                    <el-col :span="2" class="checkbox">&nbsp;&nbsp;</el-col>
                    <el-col :span="10" class="course-info">
                        <img :src="value.course_img" alt="">
                        <span class="course_name">{{value.name}}<br>
                        <span class="discount">{{value.active_name}}</span>
                        </span>

                    </el-col>
                    <el-col :span="8"><span>{{value.expire_text}}</span></el-col>
                    <el-col :span="4" class="course-price">原价¥：{{value.price}}<br><span>现价¥：{{value.true_price}}</span>
                    </el-col>
                </el-row>
            </div>
            ...
            <div class="calc">
                <el-row class="pay-row">
                    ...
                    <el-col :span="8">
                        ...
                        <span class="alipay wechat"><img src="../../static/image/logo.png" alt=""></span>
                    </el-col>
                    <el-col :span="8" class="count">实付款:<span>{{total.toFixed(2)}}</span></el-col>
                    <el-col :span="4" class="cart-pay"><span @click="generate_order">支付宝支付</span></el-col>
                </el-row>
            </div>
        </div>
        <Footer/>
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            // 获取订单页面
            order_goods_page() {
                this.$axios.get(`${this.$settings.HOST}shoppingcart/order/`, {
                    headers: {
                        'Authorization': 'jwt ' + this.token
                    }
                }).then(success => {
                    console.log(success.data)
                    this.course = success.data.data_list
        ...
        created() {
            this.token = this.ckeck_user_login()
            this.order_goods_page()
        }
    }
</script>
